...
import gotools
from som.src.io_helper import read_tsp
from som.src import distance
...
#         p = points
#         r = np.roll(points, 1, axis=0)
#
#         print()
#         return np.sum(distances)
#     loc = "C:\\Users\\zmajstorovic\\Documents\\QL_TSP\\som\\src\\fi10639.tsp"
#     data_points = read_tsp(loc)
#     dist = route_distance(data_points)
#     print(data_points)
#
#
...
#     if solution:
#         catcher = print_solution(manager, routing, solution)
#
#     return catcher
...
def main(pr_size, algo):
    """Entry point of the program."""
    # Instantiate the data problem.
    data = create_data_model(pr_size)
    ...
    # Setting first solution heuristic.
    search_parameters = pywrapcp.DefaultRoutingSearchParameters()
    if algo is 'Christofides':
        search_parameters.first_solution_strategy = (
             routing_enums_pb2.FirstSolutionStrategy.CHRISTOFIDES)
    if algo is 'SimAnnealing':
        search_parameters.first_solution_strategy = (
             routing_enums_pb2.LocalSearchMetaheuristic.SIMULATED_ANNEALING)
    if algo is 'GlobalCheapest':
        search_parameters.first_solution_strategy = (
    ...
    # Print solution on console.
    if solution:
        c, d = print_solution(manager, routing, solution)
    # print(f'ovo {d}')
    f = c.split('\n')
    indeksi = f[1].replace(' -> ', ',').split(',')

    nx_net(data['locations'], indeksi, algo)
